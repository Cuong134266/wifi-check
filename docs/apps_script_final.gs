...
function initSheets(force) {
  ...
  _ensureSheet(SHEET_QR_SCAN_RAW, [
    "Timestamp",
    "Date",
    "Type",
    "Raw_JSON",
  ...
  if (setSheet.getLastRow() <= 1) {
    [
      [
        "office_wifi_ssid",
        "UX TEAM 04, UX TEAM 04 5G, UX TEAM 06, UX TEAM 00, MB-Guest",
      ],
      [
        "office_wifi_bssid",
        "3c:52:a1:e9:af:f3, 3c:52:a1:e9:af:f2, 40:ee:15:e3:2f:d8",
      ],
...
function loginEmployee(params) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (params.name && data[i][1] !== params.name)
      sheet.getRange(i + 1, 2).setValue(params.name);
    if (params.avatar && data[i][2] !== params.avatar)
      sheet.getRange(i + 1, 3).setValue(params.avatar);
    if (params.device_id)
  ...
  return {
    success: false,
    error: "Email không có trong danh sách nhân viên. Liên hệ admin.",
  };
}
...
function checkinEmployee(params) {
  ...
  // TỰ ĐỘNG CẬP NHẬT IP MẠNG VĂN PHÒNG KHI ADMIN CHECK-IN TẠI VĂN PHÒNG
  let autoUpdatedIp = false;
  const publicIpParam = (params.public_ip || "").toString().trim();
  if (employee.role === "admin" && publicIpParam.length > 0 && !isSystemAuto) {
    const currentOfficeIp = (settings.office_public_ip || "").toString().trim();
    const validIps = currentOfficeIp
      .split(/[,;\n\r]+/)
  ...
  return {
    success: true,
    auto_updated_ip: autoUpdatedIp,
    new_ip: autoUpdatedIp ? publicIpParam : undefined,
    checkin: {
      date: today,
      time: timeStr,
      status: status,
...
function createQrSession(params) {
  ...
  const canIssue =
    employee.role === "admin" || _hasCheckedInToday(email, today, tz);
  if (!canIssue)
    return {
      success: false,
      error: "Chỉ admin hoặc người đã check-in hôm nay mới được xuất QR.",
    };
  ...
  SpreadsheetApp.getActiveSpreadsheet()
    .getSheetByName(SHEET_QR_SESSIONS)
    .appendRow([
      tokenId,
      tokenHash,
      email,
...
function checkinByQr(params) {
  ...
  if (session.status !== "active")
    return { success: false, error: "QR đã bị khóa hoặc hết hiệu lực." };
  ...
  if (
    !issuer ||
    (issuer.role !== "admin" &&
      !_hasCheckedInToday(session.issuer_email, today, tz))
  ) {
    return {
      success: false,
      error: "Người xuất QR chưa đủ điều kiện hoặc không còn hợp lệ.",
    };
  }
...
function createLeaveRequest(params) {
  ...
  if (!startDate || !endDate)
    return { success: false, error: "Ngày nghỉ là bắt buộc" };
  initSheets();
  ...
  SpreadsheetApp.getActiveSpreadsheet()
    .getSheetByName(SHEET_LEAVE_RAW)
    .appendRow([
      now.toISOString(),
      startDate,
      email,
...
function getLeaveRequests(params) {
  ...
  return {
    success: true,
    requests: _readLeaveRequests({ status: params.status || "" }),
  };
}
...
function cancelLeaveRequest(params) {
  ...
  if (!email || !requestId)
    return { success: false, error: "Email and request_id required" };
  initSheets();
  syncLeaveRawToRequests();

  const sheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_LEAVE_REQUESTS);
  const data = sheet.getDataRange().getValues();
  for (let i = 1; i < data.length; i++) {
    if (
      data[i][0] === requestId &&
      data[i][1].toString().toLowerCase() === email
    ) {
      if (data[i][8] !== "pending")
        return { success: false, error: "Chỉ hủy được đơn đang chờ duyệt" };
...
function getHistory(params) {
  ...
  const rawData = SpreadsheetApp.getActiveSpreadsheet()
    .getSheetByName(SHEET_RAW)
    .getDataRange()
    .getValues();
  const earliestByDay = {};
  ...
  for (let i = 1; i < rawData.length; i++) {
    ...
    try {
      ...
      if (
        ...
        timeStr < earliestByDay[rowDate].checkin_time
      ) {
        earliestByDay[rowDate] = {
          date: rowDate,
          checkin_time: timeStr,
  ...
  const records = Object.values(earliestByDay).sort((a, b) =>
  ...
  return {
    ...
    summary: {
      ...
      attendance_rate:
        records.length > 0
          ? Math.round((onTimeDays / records.length) * 100)
          : 0,
    },
  };
}
...
function getTodayStatus(params) {
  ...
  return Object.assign(
    { success: true },
    _todayStatus(email, getSettingsMap()),
  );
}
...
function getRanking(params) {
  ...
  const prefix = `${targetYear}-${String(targetMonth).padStart(2, "0")}`;
  const empMap = {};
  const empData = SpreadsheetApp.getActiveSpreadsheet()
  ...
  const ranking = Object.values(empMap)
    ...
    .sort((a, b) => {
      if (b.total_late_minutes !== a.total_late_minutes)
  ...
  return {
    ...
    year: targetYear,
    ranking: ranking,
    total_employees: Object.keys(empMap).length,
  };
}
...
function addEmployee(params) {
  ...
  try {
    CacheService.getScriptCache().remove("emp_" + email);
  } catch (_) {}
...
function registerDevice(params) {
  ...
  if (!email || !deviceId)
    return { success: false, error: "Email and device_id required" };
  initSheets();
  return _registerDeviceIfMissing(
    email,
    deviceId,
    params.device_model,
    params.device_os,
...
function resetDevice(params) {
  ...
  initSheets();
  const sheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_DEVICES);
  const data = sheet.getDataRange().getValues();
  let count = 0;
  for (let i = 1; i < data.length; i++) {
  ...
  return {
    success: true,
    message: `Reset ${count} device(s) for ${targetEmail}`,
  };
}
...
function _updateSingleSetting(key, value) {
  initSheets();
  const sheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_SETTINGS);
  const data = sheet.getDataRange().getValues();
...
function getStats(params) {
  ...
  const totalCheckins = ranking.ranking.reduce(
    (sum, r) => sum + r.total_days,
    0,
  );
  const totalLate = ranking.ranking.reduce((sum, r) => sum + r.late_days, 0);
  const totalLateMinutes = ranking.ranking.reduce(
...
function syncRawJsonToCheckins() {
  ...
  for (let i = 1; i < ciData.length; i++) {
    processedKeys.add(
      `${_formatSheetDate(ciData[i][2])}_${ciData[i][0].toString().toLowerCase()}`,
    );
  }
  ...
  for (let i = 1; i < rawData.length; i++) {
    ...
    const syncStatus = rawData[i][5]
      ? rawData[i][5].toString().toLowerCase()
      : "pending";
    ...
    try {
      ...
      newRows.push([
        ...
        lat,
        lng,
        obj.gps_distance !== undefined && obj.gps_distance !== ""
          ? parseFloat(obj.gps_distance).toFixed(1) + " m"
          : "",
        mapLink,
        obj.status || "",
  ...
  if (newRows.length > 0) {
    ciSheet
      .getRange(ciSheet.getLastRow() + 1, 1, newRows.length, 21)
      .setValues(newRows);
  }
  ...
  return {
    ...
    updated_raws: rowsToMarkDone.length,
  };
}
...
function syncQrScanRawToLogs() {
  ...
  for (let i = 1; i < logData.length; i++) {
    processed.add(
      String(logData[i][2] || "") +
        "_" +
        String(logData[i][5] || "").toLowerCase(),
    );
  }
  ...
  for (let i = 1; i < rawData.length; i++) {
    ...
    const syncStatus = rawData[i][4]
      ? rawData[i][4].toString().toLowerCase()
      : "pending";
    ...
    try {
      ...
      const key =
        String(obj.token_id || "") +
        "_" +
        String(obj.scanner_email || "").toLowerCase();
  ...
  if (newRows.length > 0) {
    logSheet
      .getRange(logSheet.getLastRow() + 1, 1, newRows.length, 13)
      .setValues(newRows);
  }
  rowsToMarkDone.forEach((rowIdx) =>
    rawSheet.getRange(rowIdx, 5).setValue("DONE"),
  );
  return {
    success: true,
    count: newRows.length,
...
function syncLeaveRawToRequests() {
  ...
  if (newRows.length > 0) {
    requestSheet
      .getRange(requestSheet.getLastRow() + 1, 1, newRows.length, 15)
      .setValues(newRows);
  }
  rowsToMarkDone.forEach((rowIdx) =>
    rawSheet.getRange(rowIdx, 6).setValue("DONE"),
...
function _registerDeviceIfMissing(email, deviceId, deviceModel, deviceOs) {
  ...
  const settings = getSettingsMap();
  const allowMulti =
    settings.allow_multi_device === "true" ||
    settings.allow_multi_device === "TRUE";

  for (let i = 1; i < data.length; i++) {
    if (
      data[i][0].toString().toLowerCase() === email &&
      data[i][1] === deviceId
    ) {
      return {
        success: true,
  ...
  if (!allowMulti) {
    for (let i = 1; i < data.length; i++) {
      if (
        ...
        return {
          success: false,
          error: "Tài khoản đã được liên kết với thiết bị khác.",
        };
      }
    }
  }
...
function _verifyWifi(params, settings) {
  ...
  if (officeSsid) {
    const validSsids = officeSsid
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
    if (
      validSsids.length > 0 &&
      validSsids.indexOf(params.wifi_ssid || "") === -1
    ) {
      return {
        success: false,
        error: `WiFi không đúng. Yêu cầu: ${officeSsid}, Hiện tại: ${params.wifi_ssid || "Không kết nối"}`,
      };
    }
  }
  if (officeBssid && params.wifi_bssid) {
    const validBssids = officeBssid
      .split(",")
      .map((s) => s.trim().toLowerCase())
      .filter(Boolean);
    if (
      validBssids.length > 0 &&
      validBssids.indexOf(params.wifi_bssid.toLowerCase()) === -1
    ) {
      return { success: false, error: "BSSID router không khớp." };
    }
  }
  if (officeIpPrefix && params.ip_address) {
    const validPrefixes = officeIpPrefix
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
    if (
      validPrefixes.length > 0 &&
      !validPrefixes.some((prefix) => params.ip_address.startsWith(prefix))
    ) {
      return {
        success: false,
        error: "IP address không thuộc mạng nội bộ công ty.",
      };
    }
  }
  return { success: true };
...
function _verifyDevice(params, settings) {
  ...
  const data = SpreadsheetApp.getActiveSpreadsheet()
    .getSheetByName(SHEET_DEVICES)
    .getDataRange()
    .getValues();
  for (let i = 1; i < data.length; i++) {
    if (
      data[i][0].toString().toLowerCase() ===
        params.email.toString().toLowerCase() &&
      data[i][1] === params.device_id &&
      data[i][5] === "active"
    ) {
      return { success: true };
    }
  ...
  return {
    success: false,
    error: "Thiết bị chưa được đăng ký. Liên hệ admin.",
  };
}
...
function _verifyGps(params, settings) {
  ...
  if (!params.latitude || !params.longitude)
    return { success: false, error: "Cần cấp quyền vị trí để check-in." };
  const lat = parseFloat(params.latitude);
  const lng = parseFloat(params.longitude);
  if (!lat || !lng)
    return { success: false, error: "Tọa độ GPS không hợp lệ." };
  const dist = _haversineDistance(lat, lng, officeLat, officeLng);
  if (dist > officeRadius) {
    return {
      success: false,
      error: `Bạn đang ở ngoài phạm vi công ty (${Math.round(dist)}m / ${Math.round(officeRadius)}m).`,
    };
  }
...
function _todayStatus(email, settings) {
  ...
  return found
    ? {
        checked_in: true,
        checkin_time: found.checkin_time,
        status: found.status,
      }
    : { checked_in: false };
}
  ...
  for (let i = rawData.length - 1; i >= 0; i--) {
    ...
    rawDate =
      rawDate instanceof Date
        ...
        : rawDate.toString();
    if (
      rawData[i][2].toString().toLowerCase() === cleanEmail &&
      rawDate === today
    ) {
      try {
        const obj = JSON.parse(rawData[i][4]);
...
function _markQrSessionUsed(row, usedCount) {
  const sheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_QR_SESSIONS);
...
function _appendQrScanLog(session, scanner, params, result) {
  ...
  SpreadsheetApp.getActiveSpreadsheet()
    .getSheetByName(SHEET_QR_SCAN_RAW)
    .appendRow([
      now.toISOString(),
      today,
      "qr_scan",
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === requestId) {
      ...
      return {
        success: true,
        message:
          status === "approved" ? "Đã duyệt đơn nghỉ" : "Đã từ chối đơn nghỉ",
      };
    }
  }
  return { success: false, error: "Không tìm thấy đơn nghỉ" };
...
function _readLeaveRequests(filter) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (filter.email && row.email.toString().toLowerCase() !== filter.email)
      continue;
  ...
  if (workDays) {
    ...
    if (!daysArr.includes(dayOfWeek)) {
      return {
        success: true,
        message:
          "Hôm nay không phải ngày làm việc (nằm ngoài work_days), hệ thống bỏ qua.",
      };
    }
  }
...
function processLeavesFromRawSheet() {
  ...
  if (!rawSheet || !targetSheet) {
    Logger.log(
      "Không tìm thấy một trong hai sheet. Vui lòng kiểm tra lại tên.",
    );
    return {
      success: false,
      error: "Không tìm thấy sheet LeaveRequestsRaw hoặc Đăng ký nghỉ",
    };
  }
  ...
  if (newRows.length > 0) {
    targetSheet
      .getRange(targetSheet.getLastRow() + 1, 1, newRows.length, 5)
      .setValues(newRows);
  ...
  return {
    success: true,
    count: newRows.length,
    processed: rowsToMarkDone.length,
  };
}
